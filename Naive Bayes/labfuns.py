...
def fetchDataset(dataset='iris'):
    if dataset == 'iris':
        X = np.genfromtxt('irisX.txt', delimiter=',')
        y = np.genfromtxt('irisY.txt', delimiter=',', dtype=int) - 1  # 修改这里
        pcadim = 2
    elif dataset == 'wine':
        X = np.genfromtxt('wineX.txt', delimiter=',')
        y = np.genfromtxt('wineY.txt', delimiter=',', dtype=int) - 1  # 修改这里
        pcadim = 0
    elif dataset == 'olivetti':
        X = np.genfromtxt('olivettifacesX.txt', delimiter=',')
        X = X / 255
        y = np.genfromtxt('olivettifacesY.txt', delimiter=',', dtype=int)  # 修改这里
        pcadim = 20
    elif dataset == 'vowel':
        X = np.genfromtxt('vowelX.txt', delimiter=',')
        y = np.genfromtxt('vowelY.txt', delimiter=',', dtype=int)  # 修改这里
        pcadim = 0
    else:
